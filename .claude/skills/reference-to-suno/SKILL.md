...
description: 레퍼런스 곡 하나로 Suno에 바로 쓸 트랙을 만든다. 질의문답으로 (1) 어떤 곡 (2) 원곡 가사 (3) 컨셉·언어를 차례로 묻고, C키 변환·Suno 프롬프트·송폼별 음절 일치 가사를 만들어 MCP로 SongFlow에 기록한다. "레퍼런스로 Suno 준비", "이 곡으로 곡 만들어줘", "suno 준비"에 사용.
---
...
**2. Suno style 텍스트** — 장르/무드/프로덕션 + BPM + C키 + 보컬 스타일 한 문단. 보컬 선명도 위해 "clear lead vocal, vocal-forward mix, uncluttered verses" 류 포함. **끝에 권장 설정 한 줄 추가**(곡 성격에 ±):
`[Suno settings] Weirdness 10~25% · Style Influence 50~65% · Audio Influence off(오디오 업로드 시 40~55%) · expected: <한 줄 예상>`
(Weirdness 낮을수록 보컬 안정, Style 과하면 반주가 보컬을 덮음)
...
4. `save_agent_response { trackId, task:"generate_suno_prompts", rawText }`
   - `rawText`는 **`{"style":"...","lyrics":"..."}` JSON 문자열**이어야 PromptLab에 등록됨. style에 위 `[Suno settings]` 줄 포함. lyrics는 `[Verse]`/`[Chorus]` 등 Suno 섹션 태그 부착.
5. `save_chord_progressions { trackId, progressions:[{name, chords, key:"C", mode, bpm}, …] }`
   - C키에 맞는 코드 진행 5개를 생성해 한 번에 삽입(chords는 C 기준 표기).

**4. 마무리** — 앨범/트랙명·`trackId`, 음절 일치 스폿체크 표, 권장 Suno 설정을 안내. Key/BPM은 추정치임을 한 줄로 명시.
