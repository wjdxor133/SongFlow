...
스킬 Q&A(어떤 곡·작업 키·원곡 가사·컨셉/언어)에 답하면 SongFlow에 저장됨:
...
저장→백업 → 템포 → 트랙 생성(Drums/Bass/Chords/Pads) → 송폼대로 Arrangement에
섹션 클립 + 로케이터 배치. **드럼은 섹션 energy(후렴=강/벌스=담백/프리=빌드)를 이름으로
자동 매핑**해 초안 배치된다(`drum-song-sections.py`). Session View 상단 행은 "역할별 작업대"로 남는다.

### 반복 레버 (들어보고 말로 요청)
- "코드 진행이 다 똑같아 심심해" → 섹션별 진행 재배치 (기능화성 매핑)
- "베이스가 초보 같아" → 세션 플레이어 패스 (어프로치 노트, A/A' 변주, 고스트)
- "기계 티 나" → 휴먼라이저 패스 (스윙, 포켓, 벨로시티 지터)
- 🔜 퍼커션(콩가/셰이커) 별도 트랙·트레시요 패턴은 아직 백로그(오너 수동)
...
| 트랙 | 악기 | 팁 |
|------|------|-----|
| Drums | Drum Rack (킥 C1/스네어 D1/햇 F#1 표준) | 노트는 자동 배치됨 — 킷만 확인(셋업 시 `load_drum_kit` 시도, 실패 시 직접 올리기) |
| Bass | 신스 베이스 | **Mono + Glide 20~40ms** 필수, Saturator 권장 |
| Chords | 피아노/키즈/플럭 (어택 있는 것) | |
| Pads | 패드 (어택 느린 것) | |
| (Perc) | 퍼커션 킷 🔜 | 별도 트랙·패턴은 아직 백로그 — 필요하면 손으로 트랙 추가(셰이커=F#1, 콩가=C1·D1) |
